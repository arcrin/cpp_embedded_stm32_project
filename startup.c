...
// Global variable to store the stack frame
volatile HardFaultStackFrame* hardFaultStackFrame;


void HardFault_Handler(void) {
    __asm volatile (
        "TST lr, #4 \n"           // Test EXC_RETURN bit 2 (if set, return to Thread mode with PSP)
        "ITE EQ \n"
        "MRSEQ r0, MSP \n"        // if equal (0), use MSP
        "MRSNE r0, PSP \n"        // if not equal (1), use PSP
        "B hard_fault_handler_c \n"
    );
}

void hard_fault_handler_c(HardFaultStackFrame* stackFrame) {
    // Store the stack frame globally
    hardFaultStackFrame = stackFrame;

    // Debugging: You can inspect the stack frame here or log it
    while (1); // Stay here to allow debugger connection
}
